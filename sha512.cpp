...
std::string sha512(const std::string &input, unsigned int &digest_length)
{
    EVP_MD_CTX *context = EVP_MD_CTX_new();
    if (context == nullptr)
    {
        throw std::runtime_error("Failed to create SHA512 context");
    }
...
int main()
{
    ...
    unsigned int dummy_length; // Dummy variable to avoid error
    sha512(input, dummy_length);
    ...
    for (int i = 0; i < NUM_TESTS; ++i)
    {
        ...
        std::string hash = sha512(input, digest_length);
